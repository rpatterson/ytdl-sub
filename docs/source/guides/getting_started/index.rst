...
Soon, it's time to start configuring ``ytdl-sub``. We provide a :doc:`./quick_start`_
with rigid, rote instructions on how to get a minimal configuration up and running, but
if that serves all your needs, then you're probably better off with `one of the more
user-friendly yt-dlp wrappers available`_. As a lower level tool with no GUI, most
``ytdl-sub`` users will need to understand at least some of how ``ytdl-sub`` works, how
it "thinks". So before you start configuring ``ytdl-sub``, `read on <architecture>`_ to
learn how ``ytdl-sub`` works.
...
For the most users, ``ytdl-sub`` works as follows:

Subscriptions use presets
^^^^^^^^^^^^^^^^^^^^^^^^^

Run ``$ ytdl-sub sub`` to read `a subscription file`_ that defines what `subscriptions`_
to download and place into your media library. Each subscription selects which
`presets`_ to apply. Those presets configure how each subscription is downloaded and
placed in the media library.

Presets configure plugins
^^^^^^^^^^^^^^^^^^^^^^^^^

`A preset`_ is effectively a set of plugin configurations. Specifically, a preset
consists of:
...
subscription. `The plugins`_ control how to run ``yt-dlp``, which media in the
subscription to download, how to collect and format metadata for those media, and how to
place the resulting files into your media library, and more.
...
User define additional presets in `their configuration file`_ that they then use in most
of their subscriptions. Most user-defines presets extend `the pre-built presets`_
provided by ``ytdl-sub``.
...
processing. Overrides are actually just another plugin configuration. The subscriptions
file has special support for `overriding the presets of all subscriptions in the
file`_. The configuration file supports `a few special options`_ that are not about
defining presets. See `the reference documentation`_ for technically complete details,
but for almost all of the use cases served by ``ytdl-sub``, the above is accurate and
representative.
